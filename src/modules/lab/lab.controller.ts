...
export class LabController {
  ...
  createOrder(@Body() dto: CreateLabOrderDto, @Req() req) {
  ...
  collect(@Param('id') id: string, @Body() dto: CollectSampleDto, @Req() req) {
  ...
  releaseResult(@Param('id') id: string, @Body() dto: UpdateLabResultDto, @Req() req) {
    return this.labUseCases.processResult(req.tenant.id, req.user.sub, id, dto.value);
  }

  @Post('orders/:id/sign')
  @RequirePermissions({ module: 'exames', action: 'liberar' })
  sign(@Param('id') id: string, @Body() dto: SignReportDto, @Req() req) {
